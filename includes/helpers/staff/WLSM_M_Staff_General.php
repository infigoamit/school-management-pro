...
class WLSM_M_Staff_General {
	...
	public static function fetch_student( $school_id, $session_id, $id, $restrict_to_section = false ) {
		...
		$student = $wpdb->get_row( $wpdb->prepare( 'SELECT sr.ID, sr.name as student_name, sr.gender, sr.phone, sr.email, sr.address, sr.city, sr.state, sr.country, sr.religion, sr.caste, sr.blood_group, sr.dob, sr.father_name, sr.father_phone, sr.father_occupation, sr.mother_name, sr.mother_phone, sr.mother_occupation, sr.admission_number, sr.enrollment_number, sr.fees_type_list, sr.admission_date, sr.photo_id, sr.id_number, sr.id_proof, sr.parent_id_proof, sr.note, sr.route_vehicle_id, sr.parent_user_id, c.ID as class_id, c.label as class_label, se.ID as section_id, se.label as section_label, se.class_school_id, sr.roll_number, sr.is_active, ss.label as session_label, u.user_email as login_email, u.user_login as username, tf.from_student_record, tf.to_school FROM ' . WLSM_STUDENT_RECORDS . ' as sr
		JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
		JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
		JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
