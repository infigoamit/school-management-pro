...
class WLSM_M_Staff_Examination {
	...
	public static function get_school_published_exam_admit_card( $school_id, $exam_id ) {
		global $wpdb;
		$exam = $wpdb->get_row( $wpdb->prepare( 'SELECT ex.ID, ex.label as exam_title, ex.start_date, ex.end_date FROM ' . WLSM_EXAMS . ' as ex 
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id 
		WHERE ex.school_id = %d AND ex.is_active = 1 AND ex.admit_cards_published = 1 AND ex.ID = %d', $school_id, $exam_id ) );
		return $exam;
	}
	...
	public static function get_exam_admit_cards( $school_id, $exam_id ) {
		global $wpdb;
		$admit_cards = $wpdb->get_results( $wpdb->prepare( 'SELECT ac.ID, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.email, c.label as class_label, se.label as section_label FROM ' . WLSM_ADMIT_CARDS . ' as ac 
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id 
	...
	public static function fetch_admit_card( $school_id, $id ) {
		global $wpdb;
		$admit_card = $wpdb->get_row( $wpdb->prepare( 'SELECT ac.ID, ac.exam_id, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.photo_id, sr.email, c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_ADMIT_CARDS . ' as ac 
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id 
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id 
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id 
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id 
	...
	public static function get_exam_group_exams( $school_id, $class_school_id, $exam_group ) {
		...
		JOIN ' . WLSM_CLASS_SCHOOL_EXAM . ' as csex ON csex.exam_id = ex.ID 
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id 
		WHERE ex.school_id = %d AND csex.class_school_id = %d AND ex.exam_group = %s ORDER BY ex.start_date ASC', $school_id, $class_school_id, $exam_group ) );
		return $exams;
	}
	...
	public static function get_student_published_exam_result( $school_id, $student_id, $admit_card_id ) {
		global $wpdb;
		$exam_result = $wpdb->get_row( $wpdb->prepare( 'SELECT DISTINCT ac.ID, ex.ID as exam_id, ex.label as exam_title, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.photo_id, sr.email, c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_EXAM_RESULTS . ' as er 
			JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id 
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id 
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id 
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id 
	...
	public static function get_exam_results( $school_id, $exam_id, $order = 'ASC' ) {
		global $wpdb;
		$exam_results = $wpdb->get_results( $wpdb->prepare( 'SELECT ac.ID, ep.exam_id, ac.roll_number, c.label as class_label, se.label as section_label, sr.name, sr.enrollment_number, COALESCE(SUM(ep.maximum_marks), 0) as total_marks, COALESCE(SUM(er.obtained_marks), 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er 
			JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id 
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id 
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id 
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id 
			...
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id 
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id 
			WHERE ex.school_id = %d AND ep.exam_id = %d GROUP BY ac.ID ORDER BY ac.roll_number ' . $order . ', sr.name', $school_id, $exam_id ) );
		return $exam_results;
	}
