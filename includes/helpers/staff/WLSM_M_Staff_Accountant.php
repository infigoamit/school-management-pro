...
class WLSM_M_Staff_Accountant {
	...
	public static function fetch_expense_query( $school_id ) {
		...
		WHERE ep.school_id = ' . absint( $school_id );
		return $query;
	}
	...
	public static function fetch_expense_query_count( $school_id ) {
	...
	public static function fetch_expense( $school_id, $id ) {
		global $wpdb;
		$expense = $wpdb->get_row( $wpdb->prepare( 'SELECT ep.ID, ep.label, ep.invoice_number, ep.amount, ep.expense_date, ep.note, ep.expense_category_id FROM ' . WLSM_EXPENSES . ' as ep
		WHERE ep.school_id = %d AND ep.ID = %d', $school_id, $id ) );
		return $expense;
	}
	...
	public static function fetch_income_query( $school_id ) {
	...
	public static function fetch_income( $school_id, $id ) {
		global $wpdb;
		$income = $wpdb->get_row( $wpdb->prepare( 'SELECT im.ID, im.label, im.invoice_number, im.amount, im.income_date, im.note, im.income_category_id FROM ' . WLSM_INCOME . ' as im
		WHERE im.school_id = %d AND im.ID = %d', $school_id, $id ) );
		return $income;
	}
	...
	public static function fetch_fee_query( $school_id ) {
		$query = 'SELECT ft.ID, ft.label, ft.amount, ft.period FROM ' . WLSM_FEES . ' as ft
