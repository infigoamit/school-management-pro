 ...
 *    $checker = new WLSM_LicenseChecker($_GET['license_key']);
 ...
 *     $checker = new WLSM_LicenseChecker(file_get_contents('license_key.php'));
...
class WLSM_LC
{
    ...
    /** @var WLSM_LicenseChecker_ActivationResponse */
     ...
     * @return \WLSM_LicenseChecker_ActivationResponse
     */
    protected function processActivationResponse($resp)
    {
        $response = new WLSM_LicenseChecker_ActivationResponse($resp);
    ...
    {
        $ret = new WLSM_LicenseChecker_ActivationResponse;
        if (empty($response))
            return $ret;
        $c = new WLSM_Crypt_Blowfish($this->_local_encryption_key);
        try {
...
class WLSM_LicenseChecker_ActivationResponse
{
    public $code;
    public $message;
...
class WLSM_Crypt_Blowfish
{
    ...
    function _init()
    {
        $defaults = new WLSM_Crypt_Blowfish_DefaultKey();
        $this->_P = $defaults->P;
        $this->_S = $defaults->S;
    }
