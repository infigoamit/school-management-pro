...
class WLSM_M_Staff_Class
{
	...
	{
		$query = 'SELECT mt.ID, mt.host_id, mt.meeting_id, mt.topic, mt.duration, mt.start_at, mt.type, mt.start_url, mt.join_url, sj.label as subject_name, sj.code as subject_code, c.label as class_label, a.name as name FROM ' . WLSM_MEETINGS . ' as mt
	...
	{
		$query = 'SELECT mt.ID, mt.host_id, mt.meeting_id, mt.topic, mt.duration, mt.start_at, mt.start_url, mt.type, mt.password, mt.join_url, sj.label as subject_name, sj.code as subject_code, c.label as class_label FROM ' . WLSM_MEETINGS . ' as mt
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = mt.school_id
		LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = mt.class_school_id
		LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		LEFT OUTER JOIN ' . WLSM_SUBJECTS . ' as sj ON sj.ID = mt.subject_id
